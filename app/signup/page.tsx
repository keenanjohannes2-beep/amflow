...
export default function SignupPage() {
  ...
  const handleSignup = async () => {
    ...
    if (data.user) {
      await supabase.from('profiles').insert({ id: data.user.id, full_name: fullName, role: 'account_manager' })
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f9f9f8' }}>
      <div style={{ background: 'white', border: '0.5px solid #e5e5e5', borderRadius: 12, padding: '40px', width: '100%', maxWidth: 380 }}>
        ...
        <p style={{ fontSize: 13, color: '#888', marginBottom: 24 }}>Start managing your clients today</p>

        <div style={{ marginBottom: 14 }}>
          <label style={{ fontSize: 12, color: '#555', display: 'block', marginBottom: 5 }}>Full name</label>
          <input
            type="text"
            value={fullName}
            onChange={e => setFullName(e.target.value)}
            placeholder="Keenan Johannes"
...
<p style={{ fontSize: 12, color: '#888', textAlign: 'center', marginTop: 20 }}>
          ...
          <a href="/login" style={{ color: '#1D9E75', textDecoration: 'none' }}>Sign in</a>
        </p>
